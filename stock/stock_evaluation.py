...
def dividends_dataframe(dividends):
    # Returns the dividends Queryset as a pandas dataframe

    if len(dividends.values()) != 0:
        df = pd.DataFrame(list(dividends.values()))
    else:
        df = pd.DataFrame(
            {
                "ticker": [],
...
def portfolio_dataframe(stocks, dividends):
    ...
    dividends_df = dividends_dataframe(dividends)
    ...
    for stock in index:
        ...
        for index, row in transactions_df[transactions_df.ticker == stock].iterrows():
            if row["type"] == "buy":
                average_local = (
                    (total * average_local) + (row["volume"] * row["price"])
                ) / (total + row["volume"])
                average = (
                    (total * average)
                    + (row["volume"] * row["price"] / row["currency_con"])
                ) / (total + row["volume"])
        ...
        if currency == "EUR":
            ...
            value_now = round(
                (total * si.get_live_price(stock)), 0
            ) / si.get_live_price(currency)
        ...
        portfolio_df.loc[stock, "Gain"] = (
            round(
                portfolio_df.loc[stock, "Total_PL"]
                / portfolio_df.loc[stock, "Total_Spent"],
                3,
            )
            * 100
        )
